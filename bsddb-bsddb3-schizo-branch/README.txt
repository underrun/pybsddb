...
to 3.1.17 up to and including DB 4.1.24.  It should also be compatible
with Python versions from 2.0 to the current alpha snapshots of Python
2.3.  Users of python 2.0 will need to install the pyunit package
(http://pyunit.sourceforge.net/) in order to run the test suite.
...
1. First, make sure that you have Berkeley DB 3.1, 3.2, 3.3, 4.0 or
   4.1 (I'm using 4.1.24 currently and have used 4.0.14 extensively)
   and that it is built and installed.  Setup.py will detect a db3 or
   BerkeleyDB directory under either /usr/local or /usr in that order;
   this will catch installations from RPMs and most hand installations
   under Unix.  If setup.py can't find your libdb then you can give it
   a hint either in the environment (using the BERKELEYDB_DIR
   evironment variable) or on the command line by specifying the
   directory containing the include and lib directory.  For example:

            --berkeley-db=/stuff/BerkeleyDB.4.1
   ...
   Check your compiler and linker documentation to be sure.  On Linux
   systems, it's just much easier to edit /etc/ld.so.conf, add
   /stuff/BerkeleyDB.4.1/lib and run ldconf (as root).
